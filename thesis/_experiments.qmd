...
The three experiments can now be compared against the qualitative rubric to make their strengths and limits explicit.
...
Several patterns emerge from this comparison. First, Suzanne is strongest when the task is either short and well-bounded (Experiment 1) or clearly grounded in recent context (Experiment 3). Second, longer workflows remain useful but place pressure on the viewport as a delivery surface (Experiment 2). Third, transparency and user control remain consistently strong across all three experiments because the interface keeps output visible, never mutates the scene automatically, and gives the user time to inspect before acting.
...
These three mappings can be stated more directly as follows.
